...
(defun realgud-locals-update-value (local-var-name)
  "Update value of single variable in frame hash and update locale buffer.

LOCAL-VAR-NAME - variable to get"
  (interactive "sVariable: ")
  (let* ((frame-id 'frame_id_placeholder)
	 (locals-data-hash (realgud-get-info 'locals-data))
	 (frame-data-hash (gethash frame-id locals-data-hash))
	 (value (car (realgud-run-command-get-output 'realgud:cmd-info-value local-var-name))))
    (puthash local-var-name
	     (car (realgud-run-command-get-output 'realgud:cmd-info-value local-var-name))
	     frame-data-hash) )
  (realgud-locals-insert) )
...
(defun realgud-locals-insert ()
  ...
  (let ((frame-data-hash
	...
	(insert variable)
	(if (gethash variable frame-data-hash)
	    (progn
	      (insert " ") (insert (gethash variable frame-data-hash))) )
	(insert "\n") )
